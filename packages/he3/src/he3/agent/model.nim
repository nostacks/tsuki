...
type
  ...
  ToastNotice* = object
    ## A short transient confirmation or failure shown outside the transcript.
    id*: string
    text*: string
    success*: bool
...
func toast*(id, text: string, success = true): AgentEvent =
  ## A transient confirmation (or failure when `success` is false) that the
  ## shell shows briefly instead of appending a transcript row.
  AgentEvent(kind: agentToast, id: id, text: text, success: success)
...
func eventFootprint(event: AgentEvent, limit: int): int =
  ...
  addString event.viewStatus.directory
  addString event.viewStatus.reasoningEffort
  for entry in event.selectorEntries:
    ...
    addString entry.defaultReasoningEffort
    for effort in entry.reasoningEfforts:
      addAmount 16
      addString effort
...
proc apply*(chat: AgentChat, event: AgentEvent) =
  ...
  of agentStatusUpdated:
    chat.status = AgentViewStatus(
      ...
      message: sanitizeText(event.viewStatus.message,
        plainTextPolicy(maxBytes = 1024)),
      directory: sanitizeText(event.viewStatus.directory,
        plainTextPolicy(maxBytes = 32 * 1024)),
      reasoningEffort: sanitizeText(event.viewStatus.reasoningEffort,
        plainTextPolicy(maxBytes = 64)),
  ...
  of agentSessionReset:
    # Emptying the journal while the revision stays monotonic forces every
    # view to rebuild: ids and versions may repeat in the new session.
    chat.items.setLen 0
    chat.usage = Usage()
    chat.plan.setLen 0
  ...
  of agentSelectorUpdated:
    ...
    for entry in event.selectorEntries:
      ...
      for effort in entry.reasoningEfforts:
        if chat.selectorEntries[^1].reasoningEfforts.len >= 16: break
        let clean = sanitizeText(effort, plainTextPolicy(maxBytes = 64))
        if clean.len > 0 and clean notin chat.selectorEntries[
            ^1].reasoningEfforts:
          chat.selectorEntries[^1].reasoningEfforts.add clean
      if entry.defaultReasoningEffort in chat.selectorEntries[
          ^1].reasoningEfforts:
        chat.selectorEntries[^1].defaultReasoningEffort = entry.defaultReasoningEffort
  ...
  of agentToast:
    chat.toast = ToastNotice(id: event.id, success: event.success,
      text: sanitizeText(event.text, plainTextPolicy(maxBytes = 512)))
    inc chat.toastSerial
